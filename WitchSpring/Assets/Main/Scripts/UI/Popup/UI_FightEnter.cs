...
public class UI_FightEnter : UI_Popup
{
    ...
    public void UpdateText()
    {
        ...
        string monsterStatString = $"체력: {GameManager.Data.GetMonsterStats("HP")}\n" +
                                   $"힘: {GameManager.Data.GetMonsterStats("STR")}\n" +
                                   $"마력: {GameManager.Data.GetMonsterStats("INT")}\n" +
                                   $"민첩: {GameManager.Data.GetMonsterStats("DEX")}\n" +
                                   $"물리방어: {GameManager.Data.GetMonsterStats("DEF")}\n" +
                                   $"마법방어: {GameManager.Data.GetMonsterStats("MDEF")}";

        nameText.text = GameManager.Data.GetMonsterName();
        infoText.text = GameManager.Data.GetMonsterInfo();
        statsText.text = monsterStatString;
    }
}
